 ...
 // G4Material * Si = man->FindOrBuildMaterial("G4_Si");
  ...
  G4Element* O = new G4Element("Oxygen"  , "O", z=8 , a=16.00*g/mole);
    ...
    G4Element* Si = new G4Element("Silicium", "Si", z=14 , a=28.086*g/mole);
     ...
     man->FindOrBuildMaterial("G4_BRASS", isotopes);
...
//    MainZ = 95.*mm;
  ...
  ShieldPhysi = new G4PVPlacement(0,			                               //no rotation
            G4ThreeVector(0.,0.,51.53*mm),	                               //at (0,0,0)
...
// Second detector - Si
  ...
  fLogicWorld->SetVisAttributes(worldVisAtt);
    ...
    lVacDet->SetVisAttributes(G4VisAttributes::Invisible);
    // Detector1 - yellow
...
//              lLowTube3 -> SetVisAttributes(Tube1VisAtt);
 ...
 //                  lCone -> SetVisAttributes(Tube1VisAtt);
  ...
  fRegion->AddRootLogicalVolume(lDet2);
  fRegion->AddRootLogicalVolume(lFoil);
  fRegion->AddRootLogicalVolume(lFoto);
...
void DetectorConstruction::ConstructSDandField()
{
  ...
  G4String Det1SDname = "Det1_SD";
  G04SensitiveDetector* Det1SD =
    new G04SensitiveDetector(Det1SDname);
  SDman->AddNewDetector( Det1SD );

  G4String Det2SDname = "Det2_SD";
  G04SensitiveDetector* Det2SD =
    new G04SensitiveDetector(Det2SDname);
  SDman->AddNewDetector( Det2SD );

  G4String Det3SDname = "Det3_SD";
  G04SensitiveDetector* Det3SD =
    new G04SensitiveDetector(Det3SDname);
  SDman->AddNewDetector( Det3SD );

  G4String Det4SDname = "Det4_SD";
  G04SensitiveDetector* Det4SD =
    new G04SensitiveDetector(Det4SDname);
