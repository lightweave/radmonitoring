...
void RunAction::EndWorker(const G4Run* run)
{
  ...
  for (i=1;i<=NOBINS;i++)
  {
  analysisManager->FillNtupleDColumn(4,0, bin_width*i/keV);
  analysisManager->FillNtupleDColumn(4,1, InitEnergy[i-1]);
...
void RunAction::CreateHistogram()
{
  ...
  //  analysisManager->CreateH1("Spec","Initial spectrum", 100, 0., 8.0*MeV);
    ...
    // Creating ntuple
    analysisManager->CreateNtuple( "all","all");
    analysisManager->CreateNtupleDColumn("E0,keV");
    analysisManager->CreateNtupleDColumn("Det1,keV");
    analysisManager->CreateNtupleDColumn("Det2,keV");
    analysisManager->CreateNtupleDColumn("Det3,keV");
    analysisManager->CreateNtupleDColumn("Det4,keV");
    analysisManager->CreateNtupleDColumn("CF");
    analysisManager->CreateNtupleDColumn("angz");
    analysisManager->FinishNtuple(1);

    // Second ntuple
    analysisManager->CreateNtuple( "trigg_d2", "trigg_d2");
    analysisManager->CreateNtupleDColumn("E0,keV");
    analysisManager->CreateNtupleDColumn("Det1,keV");
    analysisManager->CreateNtupleDColumn("Det2,keV");
    analysisManager->CreateNtupleDColumn("Det3,keV");
